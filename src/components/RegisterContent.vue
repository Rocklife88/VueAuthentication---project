      ...
      <form @submit.prevent="handleRegister" class="space-y-4">
        ...
        <div>
          ...
          <div class="text-center pt-2">
            <RouterLink to="/login" class="text-green-400 text-left text-xs">Already Registered? then <span class="text-green-300 font-bold hover:text-green-500 cursor-pointer">Login</span></RouterLink>
          </div>
        </div>
...
const handleRegister = async () => {
  ...
  errorMessage.value = '';
  try {
    await store.registerUser(username.value, password.value);
